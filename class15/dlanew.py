...
if len(argv)==1:
 exit("Need value for argument") 
npart=int(argv[1]) 
side=int(argv[2])  #Should be an odd number
steps = [(1,0),(-1,0),(0,1),(0,-1)]
grid=[[0 for x in range(side)] for y in range(side)]
grid[side/2][side/2]=1

#Enter code from Class 15 handout here

        # Stop if you are next to
for ipart in range(npart):
 ...
 # Enforce periodic boundaries
         ...
         if x==side: x=0
         if y==side: y=0
         grid[x][y]=1 #Put particle in new location  a particle
