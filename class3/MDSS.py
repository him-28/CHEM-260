...
class system:
    ...
    def Hfunction(self):
        ...
        if self.KHprint:
            Hfile=open('H.txt','w')
            print>>Hfile,'Vdel=%lf Step=%lf Vbinsum=%d'%(self.VDel,self.Step,self.Vbinsum)
            for iNVin in self.Vbinsave:
                print>>Hfile,'%d %d %d'%(iNVin.x,iNVin.y,iNVin.z)
            Hfile.close()
            #self.Hprint=0
    ...
    def radialdist(self,stepnumber):
        ...
        print>>self.rdffile,'#Radial Dist Function at time=%d'%(stepnumber)
        print>>self.rdffile,'#Natom=%d, Density=%lf, Ave Temp=%lf'%(self.NAtom, self.Density, self.TempAve)
        print>>self.rdffile,'#R  G(r)'
        print>>self.rdffile,'@  title \"Mean square displacement\"'
        print>>self.rdffile,'@  xaxis label \"Radius (sigma)"'
        print>>self.rdffile,'@  yaxis label \"Particle density\"'
        ...
        for i in range(self.NRDels):
            ...
            if (self.NRadialDist[i]>0):
                ...
                print>>self.rdffile,'%7.3lf %8.5lf'\
                                    %(Radius,self.RadialDist[i])
        self.rdffile.close()

    ### Functions related to printing .pdb and .crd trajectory files ###
    def printpdb(self):
        pdbfile=open('ar.pdb','w')
        print>>pdbfile,'COMMENT SYSTEM OF ARGON ATOMS'
        icount=1
        for iatom in self.atoms:
            if self.labelatom and icount==1:
                print>>pdbfile,'ATOM  %5d  AR               %8.3lf%8.3lf%8.3lf'%(icount,iatom.pos.x,iatom.pos.y,iatom.pos.z)
            else:
                print>>pdbfile,'ATOM  %5d  AR               %8.3lf%8.3lf%8.3lf'%(icount,iatom.pos.x,iatom.pos.y,iatom.pos.z)
            icount+=1
        pdbfile.close()
    ...
    def startcrd(self):
        ...
        print>>self.crdfile,'CRD FILE ARGON ATOMS'
        print>>self.crdfile,'%5d'%(self.NAtom)
        for i in range(0,self.NAtom,2):
            print>>self.crdfile,'%12.7lf%12.7lf%12.7lf%12.7lf%12.7lf%12.7lf'%(self.atoms[i].pos.x,
                                                                                self.atoms[i].pos.y,
                                                                                self.atoms[i].pos.z,
                                                                                self.atoms[i+1].pos.x,
                                                                                self.atoms[i+1].pos.y,
                                                                                self.atoms[i+1].pos.z)
    def printcrd(self):
        for i in range(0,self.NAtom,2):
            print>>self.crdfile,'%12.7lf%12.7lf%12.7lf%12.7lf%12.7lf%12.7lf'%(self.atoms[i].pos.x,
                                                                                   self.atoms[i].pos.y,
    ...
    def printvelautocorr(self):
        ...
        print>>velautofile,'#Velocity Autocorrelation Function'
        print>>velautofile,'#Time     <r2>'
        print>>velautofile,'@  title \"Velocity Autocorrelation Function\"'
        print>>velautofile,'@  xaxis label \"Time (reduced time units)\"'
        print>>velautofile,'@  yaxis label \"V*V\"'
        for i in range(self.velautolen):
            print>>velautofile,'%12.8lf %12.8lf'%(i*self.Step,self.velautocorrave[i])
        velautofile.close()
    ...
    def printviscautocorr(self):
        ...
        print>>viscautofile,'#Mean square displacement of all atoms'
        print>>viscautofile,'#Time     <r2>'
        print>>viscautofile,'@  title \"Mean square displacement\"'
        print>>viscautofile,'@  xaxis label \"Time (reduced time units)\"'
        print>>viscautofile,'@  yaxis label \"J*J\"'
        for i in range(self.Jautolen):
            print>>viscautofile,'%12.8lf %12.8lf'%(i*self.Step,self.Jautocorr[i])
        viscautofile.close()
    ...
    def printvels(self):
        if self.openvelfile:
            ...
            self.openvelfile=0
        for iatom in self.atoms:
            print>>self.velfile,'%12.8lf %12.9lf %12.8lf'%(iatom.vel.x/self.Step,iatom.vel.y/self.Step,iatom.vel.z/self.Step)
